...
func TestIntegrationTests(t *testing.T) {
	...
	runIntegrationTest(t, "testAllocateStaticAddress", testAllocateStaticAddress)
}
...
func testAllocateStaticAddress(ctx context.Context, t *testing.T, md integrationTestMetadata, service *vpcService, session *ec2wrapper.EC2Session) {

	req := &titus.AllocateAddressRequest{
		AddressAllocation: &titus.AddressAllocation{
			AddressLocation: &titus.AddressLocation{

				Region:           md.region,
				AvailabilityZone: md.az,
				SubnetId:         md.subnetID,
			},
		},
		AccountId: md.account,
	...
	resp, err := service.AllocateAddress(ctx, req)
	assert.NilError(t, err)
	fmt.Println(resp)
}
