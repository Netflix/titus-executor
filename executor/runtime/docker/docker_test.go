...
func testEnvFileTemplate(t *testing.T, envVars map[string]string, expected string) {
	...
	assert.NoError(t, executeEnvFileTemplate(&runtimeTypes.Container{Env: envVars}, imageInspect, &buf))
	assert.Equal(t, expected, buf.String())
}

func TestEnvFileTemplate(t *testing.T) {
	env := map[string]string{"NORMAL": "normal"}
	testEnvFileTemplate(t, env, `# This file was autogenerated by the titus executor
...
func TestEnvFileDocker(t *testing.T) {
	...
	assert.NoError(t, executeEnvFileTemplate(&runtimeTypes.Container{
		Env: map[string]string{"A": "TEST1", "B": "TEST2", "FOO": "baz"},
	}, imageInspect, &buf))
