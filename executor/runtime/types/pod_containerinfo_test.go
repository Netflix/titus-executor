...
func TestNewPodContainerInfoContainer(t *testing.T) {
	...
	c, err := NewPodContainerInfoContainer(pod, &ipAddr)
	assert.NilError(t, err)
...
func TestNewPodContainerInfoErrors(t *testing.T) {
	ipAddr := ""
	_, err := NewPodContainerInfoContainer(nil, &ipAddr)
	...
	_, err = NewPodContainerInfoContainer(pod, nil)
	assert.Error(t, err, "missing ipv4 address")

	_, err = NewPodContainerInfoContainer(pod, &ipAddr)
...
func TestNewPodContainerInfoHostnameStyle(t *testing.T) {
	pod := &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Name: taskID,
		},
