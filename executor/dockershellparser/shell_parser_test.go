/*
 * This file is derived from the github.com/docker/docker/builder/dockerfile/shell_parser
 * package. That file was released under the Apache v2.0 License.
 * TODO(Andrew L): We should just import that package as a dependency rather than including
 * this file. Including that dependency brought in some other issues that need to be resolved.
 */
package dockershellparser
...
func TestShellParser4EnvVars(t *testing.T) {
	...
	defer func() {
		if err := file.Close(); err != nil {
			t.Fatalf("Failed to close %s: %s", file.Name(), err)
		}
	}()
...
func TestShellParser4Words(t *testing.T) { // nolint: gocyclo
	fn := "wordsTest"
	...
	defer func() {
		if err := file.Close(); err != nil {
			t.Logf("Failed to close %s: %s", file.Name(), err)
		}
	}()
